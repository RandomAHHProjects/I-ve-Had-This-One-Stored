...
function DeltaBXC2.new(title, size)
    ...
    local tabList = new("UIListLayout", {Parent = sidebar, SortOrder = Enum.SortOrder.LayoutOrder})
    ...
    function self:CreateTab(name)
        ...
        function tab:CreateSelector(label, list, callback)
            ...
            new("UICorner", {Parent = label, CornerRadius = UDim.new(0, 6)})
    ...
    function self:SaveToFile(name)
        -- stub: use writefile(name, HttpService:JSONEncode(self._data)) later
    end

    function self:LoadFromFile(name)
        -- stub: load JSON, decode, restore states later
    end
